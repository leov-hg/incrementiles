...
{
    public class FacebookImplementation : IAnalytics
    {
        ...
        public void TrackInAppPurchaseEvent(string productId, string currencyCode, double unitPrice, string transactionId = null, string payload = null)
        {
            if (FB.IsInitialized)
            {
                ...
                FB.LogPurchase(Convert.ToDecimal(unitPrice), currencyCode, parameters);
            }
            else
            {
                ...
                delayedActionsUntilInitialization.Push(() =>
                {
                    TrackInAppPurchaseEvent(productId, currencyCode, unitPrice, transactionId, payload);
                });
            }
        }
        ...
        public void TrackProgressionEvent(ProgressionStatus progressionStatus, string progression, int score = 0)
        {
            ...
            AddParameterToDictionary(parameters, AppEventParameterName.ContentID, progression);
