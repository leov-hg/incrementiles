        ...
        public static async Task<RemoteConfigurationSetup> FetchRemoteConfiguration()
        {
            ...
            try
            {
                ...
                if (trackingData != null && trackingData.ContainsKey("ti"))
                {
                    ...
                    string trackingDataString = trackingData.ContainsKey("dp") && !string.IsNullOrEmpty((string)trackingData["dp"]) ? UnityWebRequest.EscapeURL((string) trackingData["dp"]) : "";
                    string everyTimeUri = string.Format(RemoteConfigurationConstants.API_EVERY_TIME_URL, trackingData["ti"], GetUserAgent(), trackingDataString);
