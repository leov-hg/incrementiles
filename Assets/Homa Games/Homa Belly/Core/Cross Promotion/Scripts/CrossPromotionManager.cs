...
namespace HomaGames.HomaBelly
{
    public class CrossPromotionManager
    {
        ...
        private static RemoteConfiguration.RemoteConfigurationSetup remoteConfigurationSetup;
        ...
        /// </summary>
        /// <param name="remoteConfigurationSetup">The Remote Configuration fetched from server</param>
        public static void Initialize(RemoteConfiguration.RemoteConfigurationSetup configurationSetup)
        {
            CrossPromotionVideoPath = Application.persistentDataPath;
            remoteConfigurationSetup = configurationSetup;
        ...
        {
            CrossPromotionItem nextItemToDownload = remoteConfigurationSetup.CrossPromotionConfigurationModel.RandomElementByWeightToBeDownloaded();
            // If an item to be downloaded is found
            if (nextItemToDownload != null)
            {
                // Trigger and wait for download. Then fulfill queue
                videoDownloader.DownloadItem(nextItemToDownload).ContinueWith((result) =>
                {
        ...
        private static void FillQueueWithWeightedSelectedItems()
        {
            ...
            Task.Run(() =>
            {
                while (CrossPromotionItemsConcurrentQueue.Count < MIN_ITEMS_IN_QUEUE)
                {
                    try
                    {
                        CrossPromotionItem item = remoteConfigurationSetup.CrossPromotionConfigurationModel.RandomElementByWeightAndLocalAvailability();
            ...
            }).ContinueWith((t)=>InitializationFinished(),TaskScheduler.FromCurrentSynchronizationContext());
        }
        ...
        {
            return remoteConfigurationSetup != null
                && remoteConfigurationSetup.CrossPromotionConfigurationModel != null
                && remoteConfigurationSetup.CrossPromotionConfigurationModel.Items != null
                && remoteConfigurationSetup.CrossPromotionConfigurationModel.Items.Count > 0;
        }
        /// <summary>
