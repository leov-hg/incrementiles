...
public class ColorPicker : MonoBehaviour
{
    ...
    [SerializeField] private bool shaderModif = false;

    [Header("Event")]
    public ColorChangedEvent onValueChanged = new ColorChangedEvent();
    public HSVChangedEvent onHSVChanged = new HSVChangedEvent();

    public bool gradientColor1 = false;
    public bool gradientColor2 = false;
    ...
    private void Awake()
    {
        if (shaderModif)
        {
            Shader.SetGlobalColor("GLOBAL_KebabColor", Color.white);
        }
    }
    ...
    private void SendChangedEvent()
    {
        if (shaderModif)
        {
            if (gradientColor1)
            {
                Shader.SetGlobalColor("GLOBAL_Color", CurrentColor);
            }
            else if (gradientColor2)
            {
                Shader.SetGlobalColor("GLOBAL_Color2", CurrentColor);
            }
        }
