...
public class DynamicBone : MonoBehaviour
{
    ...
    public void SetPlayersColliders()
    {
        //Set Players colliders
        int playerNumber = GameObject.FindGameObjectsWithTag("Player").Length;
        List<DynamicBoneCollider> playerBoneColliders = new List<DynamicBoneCollider>();

        Debug.Log(playerNumber);

        foreach (GameObject player in GameObject.FindGameObjectsWithTag("Player"))
        {
            playerBoneColliders.Add(player.GetComponent<DynamicBoneCollider>());
            Debug.Log(playerBoneColliders[0]);
        }

        for (int i = 0; i < playerNumber; i++)
        {
            m_Colliders.Add(playerBoneColliders[i]);
        }
    }
