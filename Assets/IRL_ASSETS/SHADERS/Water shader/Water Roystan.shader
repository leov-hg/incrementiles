Shader "Roystan/Toon/Water Tut"
{
    ...
    SubShader
    {
        ...
        Pass
        {
			...
			fixed _SurfaceNoiseCutoff;
			fixed _SurfaceDistortionAmount;

			fixed2 _SurfaceNoiseScroll;
            ...
            fixed4 frag (v2f i) : SV_Target
            {
				...
				float foamDistance = lerp(_FoamMaxDistance, _FoamMinDistance, waterDepthDifference01);
				float foamDepthDifference01 = saturate(depthDifference / foamDistance);

				float surfaceNoiseCutoff = foamDepthDifference01 * _SurfaceNoiseCutoff;
				//float surfaceNoiseCutoff = depthDifference * _SurfaceNoiseCutoff;

				fixed2 distortUV = TRANSFORM_TEX(i.uv, _SurfaceDistortion);
				fixed2 noiseUV = TRANSFORM_TEX(i.uv, _SurfaceNoise);

				float2 distortSample = (tex2D(_SurfaceDistortion, distortUV).xy * 2 - 1) * _SurfaceDistortionAmount;
				...
				float2 surfaceNoiseUV = float2((noiseUV.x + _Time.y * _SurfaceNoiseScroll.x) + distortSample.x, 
				(noiseUV.y + _Time.y * _SurfaceNoiseScroll.y) + distortSample.y);
				float surfaceNoiseSample = tex2D(_SurfaceNoise, surfaceNoiseUV).r;
				...
				//return behindCol * step(waterDepthDifference01, 1);
				//return 1 - step(depthDifference, 0);
