Shader "Custom/BlurShader"
{
	...
	SubShader 
	{
		Pass
		{
			...
			{
				//fixed2 dist = ((i.uv + 0) - 0.5);
				//dist.x = dot(dist, dist) * 20;

				fixed dist = clamp((abs((i.uv.y * 2 - 1)) - _Radius) * 20, 0, 15);

				float4 color = float4(0.0, 0.0, 0.0, 0.0);
				float2 offset1 = float2(1.3846153846, 1.3846153846) * _Dir * dist;
				float2 offset2 = float2(3.2307692308, 3.2307692308) * _Dir * dist;
