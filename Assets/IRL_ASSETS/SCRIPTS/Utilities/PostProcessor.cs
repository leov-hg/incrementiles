...
public class PostProcessor : MonoBehaviour
{
	...
	private float targetVignettePower;
	private float vignetteSmoothSpeed;
	private float randomNumber;
    private float currentTime = Mathf.Infinity;
    private bool powerVariationEnabled = false;
	...
	private void ApplyVignette(RenderTexture from, RenderTexture to) {
		if(VignetteShader != null) {
            ...
            VignetteMaterial.SetColor(ShaderUniforms.VignetteColor, VignetteColor);
    ...
    private void Update()
    {
        ...
        {
            currentTime = 0;
            randomNumber = (Random.value * 2 - 1) * 0.5f;
        }

        if (powerVariationEnabled)
        {
            VignettePower = Mathf.Lerp(VignettePower, Mathf.Clamp01(targetVignettePower + randomNumber), Time.deltaTime * vignetteSmoothSpeed);
        }
        else
        {
            VignettePower = Mathf.Lerp(VignettePower, Mathf.Clamp01(targetVignettePower), Time.deltaTime * vignetteSmoothSpeed);
        }
    }

    public void SetVignetteSmoothSpeed(float value)
    {
        vignetteSmoothSpeed = value;
    }

    public void SetVignettePowerTarget(float value)
    {
        targetVignettePower = value;
    }

    public void ActivePowerVariation(bool active)
    {
        if (active)
        {
            powerVariationEnabled = false;
        }
        else
        {
            powerVariationEnabled = false;
    ...
    private IEnumerator VignetteRandomVariation(float duration, float vignettePower)
    {
        float targetVignettePower = 0;
        float currentTime = 0;
        while (currentTime < duration)
        {
            currentTime += Time.deltaTime;
            targetVignettePower = vignettePower + (Random.value * 2 - 1) * 0.5f;
            yield return null;
        }
    }
}
